...
const 检测转换器之配置项集之一切属性之合规性 = require('./辅助工具集/检测转换器之配置项集之一切属性之合规性')
...
module.exports = function 构建一个用于将Markdown内容字符串转换为HTML字符串的转换器(配置项集 = {}) {
    ...
    return function 吴乐川的将以Markdown为内容之字符串转换为HTML内容之字符串的转换器(
        ...
        if (!应输出MarkdownIt生态工具集之原始产出以便验证之而非输出正式内容) {
            const 转换器之配置项集之一切属性均合规 = 检测转换器之配置项集之一切属性之合规性(转换器之配置项集之原始值)

            if (!转换器之配置项集之一切属性均合规 && false) { // eslint-disable-line no-constant-condition
                return ''
            }
        }
        ...
        const 转换器之配置项集 = 尽量取原始值但确保最终取值为所谓标准对象(转换器之配置项集之原始值)

        const 须在控制台打印详尽细节 = !!转换器之配置项集.须在控制台打印详尽细节

        // 下面 4 行代码用于暂时兼容旧版措辞之属性 “ 须对产出之HTML内容字符串依次按下诸内容替换规则做修订 ” 。
        if (转换器之配置项集之原始值 && 转换器之配置项集之原始值.对HTML做进一步处理之阶段 && 转换器之配置项集之原始值.对HTML做进一步处理之阶段.须对产出之HTML内容字符串依次按下诸内容替换规则做修订) {
            if (!转换器之配置项集.对HTML做进一步处理之阶段) { 转换器之配置项集.对HTML做进一步处理之阶段 = {} }
            转换器之配置项集.对HTML做进一步处理之阶段.须对产出之HTML内容字符串依次按下诸内容替换规则做修订 = 转换器之配置项集之原始值.对HTML做进一步处理之阶段.须对产出之HTML内容字符串依次按下诸内容替换规则做修订
        }
        ...
        // “ 须对产出之HTML内容字符串依次按下诸内容替换规则做修订 ” 这一属性已因措辞有语病而作废。
        // 目前仍有代码兼容之，即令其取值映射、合并到更名后的 “ 须对产出之HTML内容字符串依次遵照下列诸内容替换规则做修订 ” 属性上去。
        // 未来有空我或许会去除对该属性之兼容性处理，即彻底废弃之。
        const 须对产出之HTML内容字符串依次按下诸内容替换规则做修订 = {
            // ...完备的默认配置项集.对HTML做进一步处理之阶段.须对产出之HTML内容字符串依次遵照下列诸内容替换规则做修订,
            ...尽量取原始值但确保最终取值为所谓标准对象(
                对HTML做进一步处理之阶段.须对产出之HTML内容字符串依次按下诸内容替换规则做修订
            ),
        }

        const 须对产出之HTML内容字符串依次遵照下列诸内容替换规则做修订 = {
            ...完备的默认配置项集.对HTML做进一步处理之阶段.须对产出之HTML内容字符串依次遵照下列诸内容替换规则做修订,
            ...尽量取原始值但确保最终取值为所谓标准对象(
                对HTML做进一步处理之阶段.须对产出之HTML内容字符串依次遵照下列诸内容替换规则做修订
            ),
        }
        ...
        const {
            '1 内建现成的替换规则之名称之序列': 作用于HTML的内建现成的替换规则之名称之序列__源于旧版措辞之属性,
            '2 额外的替换规则之定义之序列': 作用于HTML的额外的替换规则之定义之序列__源于旧版措辞之属性,
        } = 须对产出之HTML内容字符串依次按下诸内容替换规则做修订

        if (Array.isArray(作用于HTML的内建现成的替换规则之名称之序列__源于旧版措辞之属性) || Array.isArray(作用于HTML的额外的替换规则之定义之序列__源于旧版措辞之属性)) {
            console.log(`“${
                彩色粉笔工具.red('须对产出之HTML内容字符串依次按下诸内容替换规则做修订')
            }” 这一属性已因措辞有语病而弃用。请改用 “${
                彩色粉笔工具.green('须对产出之HTML内容字符串依次遵照下列诸内容替换规则做修订')
            }”。`)
        }
        ...
        if (!Array.isArray(作用于HTML的内建现成的替换规则之名称之序列)) {
            if (Array.isArray(作用于HTML的内建现成的替换规则之名称之序列__源于旧版措辞之属性)) {
                作用于HTML的内建现成的替换规则之名称之序列 = 作用于HTML的内建现成的替换规则之名称之序列__源于旧版措辞之属性
            } else {
                作用于HTML的内建现成的替换规则之名称之序列 = 完备的默认配置项集.对HTML做进一步处理之阶段.须对产出之HTML内容字符串依次遵照下列诸内容替换规则做修订['1 内建现成的替换规则之名称之序列']
            }
        } else {
            if (Array.isArray(作用于HTML的内建现成的替换规则之名称之序列__源于旧版措辞之属性)) {
                作用于HTML的内建现成的替换规则之名称之序列 = 作用于HTML的内建现成的替换规则之名称之序列__源于旧版措辞之属性.concat(作用于HTML的内建现成的替换规则之名称之序列)
            }
        }

        if (!Array.isArray(作用于HTML的额外的替换规则之定义之序列)) {
            if (Array.isArray(作用于HTML的额外的替换规则之定义之序列__源于旧版措辞之属性)) {
                作用于HTML的额外的替换规则之定义之序列 = 作用于HTML的额外的替换规则之定义之序列__源于旧版措辞之属性
            } else {
                // 作用于HTML的额外的替换规则之定义之序列 = 完备的默认配置项集.对HTML做进一步处理之阶段.须对产出之HTML内容字符串依次遵照下列诸内容替换规则做修订['2 额外的替换规则之定义之序列']
                作用于HTML的额外的替换规则之定义之序列 = []
            }
        } else {
            if (Array.isArray(作用于HTML的额外的替换规则之定义之序列__源于旧版措辞之属性)) {
                作用于HTML的额外的替换规则之定义之序列 = 作用于HTML的额外的替换规则之定义之序列__源于旧版措辞之属性.concat(作用于HTML的额外的替换规则之定义之序列)
            }
        }
        ...
        if (!应输出MarkdownIt生态工具集之原始产出以便验证之而非输出正式内容) {
            ...
            if (所采用之本NPM包之Peer依赖包提供之层叠样式表文件之名称) {
                const snippetEntryOfThemingCSS = 从Peer依赖包中阻塞式获取某个主题文件之内容全文字符串(
