import type {
    类型定义之由本工具内建之层叠样式表文件之名称 as 依赖包之类型定义之内建之层叠样式表文件之名称,
    类型定义之由本工具内建之层叠样式表文件之名称 as 依赖包之类型定义之内建之层叠样式表文件之名称之凡不含文章目录者,
    类型定义之由本工具内建之层叠样式表文件之名称 as 依赖包之类型定义之内建之层叠样式表文件之名称之凡含有文章目录者,
    // 类型定义之内建之层叠样式表文件之名称 as 依赖包之类型定义之内建之层叠样式表文件之名称,
    // 类型定义之内建之层叠样式表文件之名称之凡不含文章目录者 as 依赖包之类型定义之内建之层叠样式表文件之名称之凡不含文章目录者,
    // 类型定义之内建之层叠样式表文件之名称之凡含有文章目录者 as 依赖包之类型定义之内建之层叠样式表文件之名称之凡含有文章目录者,
    类型定义之文件简易描述项字典 as 依赖包之类型定义之文件简易描述项字典,
    类型定义之获取某特定文件之完整内容字符串之函数 as 依赖包之类型定义之获取某特定文件之完整内容字符串之函数,
...
declare namespace 一枚现成的自Markdown至HTML的转换器 {
    ...
    export type 类型定义之转换器之构建器之配置项集 = {
        themesPeerPackageAllDistFileEntriesKeyingByFileNames: 依赖包之类型定义之文件简易描述项字典;
        syncGetContentStringOfOneFileOfThePeerModuleOfThemes: 依赖包之类型定义之获取某特定文件之完整内容字符串之函数;
    ...
    export type 将Markdown字符串转换为HTML字符串之转换器之配置项集 = {
        ...
        对HTML做进一步处理之阶段?: {
            ...
            所采用之由本工具内建之不含文章纲要列表之定义之层叠样式表文件之名称?: 依赖包之类型定义之内建之层叠样式表文件之名称之凡不含文章目录者;
            所采用之由本工具内建之含有文章纲要列表之定义之层叠样式表文件之名称?: 依赖包之类型定义之内建之层叠样式表文件之名称之凡含有文章目录者;
