/// <reference types="@wulechuan/css-stylus-markdown-themes" />
...
export default 一枚现成的自Markdown至HTML的转换器;

declare namespace 一枚现成的自Markdown至HTML的转换器 {
    export const 构建一个用于将Markdown内容字符串转换为HTML字符串的转换器: 类型之转换器之构建器;
    ...
    export type 类型之转换器之构建器 = (配置项集: 类型之转换器之构建器之配置项集) => 自Markdown至HTML的转换器;

    export type 类型之转换器之构建器之配置项集 = {
        // themesPeerPackageAllDistFileEntriesKeyingByFileNames: 吴乐川用于美化文章的层叠样式表集之数据类型之文件简易描述项字典;
        // syncGetContentStringOfOneFileOfThePeerModuleOfThemes: 吴乐川用于美化文章的层叠样式表集之数据类型之获取某特定文件之完整内容字符串之函数;
    ...
    export type 类型之由本工具内建之层叠样式表文件之名称 = (
    ...
    export type 类型之作用于HTML内容之内建替换规则名称 = (
        | '令所有外部链接之打开方式为 _blank'
        | '令所有原本指向 Markdown 文件之链接改为指向同名 HTML 文件'
    );

    export type 类型之作用于HTML内容之额外替换规则项 = {
        凡: string | RegExp;
        替换为: string;
    };
    ...
    export type 将Markdown字符串转换为HTML字符串之转换器之配置项集 = {
        ...
        对HTML做进一步处理之阶段?: {
            ...
            所采用之由本工具内建之不含文章纲要列表之定义之层叠样式表文件之名称?: 一枚现成的自Markdown至HTML的转换器.类型之由本工具内建之层叠样式表文件之名称;
            所采用之由本工具内建之含有文章纲要列表之定义之层叠样式表文件之名称?: 一枚现成的自Markdown至HTML的转换器.类型之由本工具内建之层叠样式表文件之名称;
            ...
            须对产出之HTML内容字符串依次按下诸内容替换规则做修订?: {
                '1 内建现成的替换规则序列'?: Array<一枚现成的自Markdown至HTML的转换器.类型之作用于HTML内容之内建替换规则名称>;
                '2 额外的替换规则序列'?: Array<一枚现成的自Markdown至HTML的转换器.类型之作用于HTML内容之额外替换规则项>;
            };
