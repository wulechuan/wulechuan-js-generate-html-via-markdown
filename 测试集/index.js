...
const 自本NPM包之根文件夹至测试集输出内容之存放文件夹之相对路径 = './测试集/诸测试之输出'
...
const 由简短文章产出之甲型风貌之变种其二之HTML内容字符串 = 本NPM包现成提供的一枚转换器(简短之文章之Markdown内容字符串, {
    须在控制台打印详尽细节: [], // 这一行应引发一条错误消息，阐明该属性取值类型不当。
    ...
    作者: '吴乐川', // 这一行应引发一条错误消息，阐明该属性并不存在于标准的配置项集当中。

    对HTML做进一步处理之阶段: {
        空格数: 4, // 这一行应引发一条错误消息，阐明该属性并不存在于标准的配置项集当中。
            ...
            '2 额外的替换规则之定义之序列': [
                {
                    ...
                    保留大小写: false, // 这一行应引发一条错误消息，阐明该属性并不存在于标准的配置项集当中。
                },
            ],
        },
    },
...
const 由简短文章产出之乙型风貌之HTML内容字符串 = 本NPM包现成提供的一枚转换器(简短之文章之Markdown内容字符串, {
    ...
    杂项: {
        控制台打印信息须改用英国话: true,
        '捣乱的值': '', // 这一行应引发一条错误消息，阐明该属性并不存在于标准的配置项集当中。
    },
})
...
阻塞式将内容写入文件(
    遵循POSIX标准拼接路径(自本NPM包之根文件夹至测试转换简短文章之输出内容之存放文件夹之相对路径, '本-npm-工具之汉语版《说明书》.html'),
    由本工具之汉语版说明书产出之HTML内容字符串
)

阻塞式将内容写入文件(
    遵循POSIX标准拼接路径(自本NPM包之根文件夹至测试转换简短文章之输出内容之存放文件夹之相对路径, '《社会主义没有辜负中国》（2021年6月7日）.html'),
    本NPM包现成提供的一枚转换器(
        阻塞式读取文件之内容('./测试集/用作测试素材的原文件之集/《社会主义没有辜负中国》（2021年6月7日）.md').toString()
    )
)

阻塞式将内容写入文件(
    遵循POSIX标准拼接路径(自本NPM包之根文件夹至测试转换简短文章之输出内容之存放文件夹之相对路径, '《中国没有辜负社会主义》（2021年6月8日）.html'),
    本NPM包现成提供的一枚转换器(
        阻塞式读取文件之内容('./测试集/用作测试素材的原文件之集/《中国没有辜负社会主义》（2021年6月8日）.md').toString()
    )
)
